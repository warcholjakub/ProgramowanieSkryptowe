...
from model.core import MoveDirection, Vector2d, PositionAlreadyOccupiedError
...
def test_for_infinite_map(
    ...
    try: infinite_map.place(animal1); assert True
    except PositionAlreadyOccupiedError: assert False
    assert infinite_map.canMoveTo(Vector2d(0, 0)) is False
    assert infinite_map.isOccupied(Vector2d(0, 0)) is True
    with pytest.raises(PositionAlreadyOccupiedError):
        infinite_map.place(animal1)
    try: infinite_map.place(animal2); assert True
    except PositionAlreadyOccupiedError: assert False
    try: infinite_map.place(animal3); assert True
    except PositionAlreadyOccupiedError: assert False
    try: infinite_map.place(animal4); assert True
    except PositionAlreadyOccupiedError: assert False
...
def test_for_map_2_2(
    ...
    try: rectangular_map_2_2.place(animal1); assert True
    except PositionAlreadyOccupiedError: assert False
    ...
    try: rectangular_map_2_2.place(animal3); assert True
    except PositionAlreadyOccupiedError: assert False
    with pytest.raises(PositionAlreadyOccupiedError):
...
def test_for_map_4_4(
    ...
    try: rectangular_map_4_4.place(animal1); assert True
    except PositionAlreadyOccupiedError: assert False
    with pytest.raises(PositionAlreadyOccupiedError):
        rectangular_map_4_4.place(animal1)
    try: rectangular_map_4_4.place(animal2); assert True
    except PositionAlreadyOccupiedError: assert False
    try: rectangular_map_4_4.place(animal3); assert True
    except PositionAlreadyOccupiedError: assert False
    with pytest.raises(PositionAlreadyOccupiedError):
